...
public sealed class ReportsController(AppDbContext db, IAutoCommentService autoComments) : ControllerBase
{
    ...
    public async Task<ActionResult> GenerateSessionComment(long id, CancellationToken ct)
    {
        ...
        return result is null ? Conflict("Session chua co summary.") : Ok(result);
    }
    ...
    public async Task<ActionResult> GenerateStudentComment(long sessionId, long studentId, CancellationToken ct)
    {
        ...
        return result is null ? Conflict("Chua co summary sinh vien.") : Ok(result);
    }
    ...
    public async Task<ActionResult> GenerateClassSectionComment(long id, CancellationToken ct)
    {
        ...
        return result is null ? Conflict("Lop chua co summary.") : Ok(result);
    }
    ...
    {
        var session = await db.SessionsSet.AsNoTracking().Include(x => x.ClassSection).ThenInclude(x => x.Course).Include(x => x.ClassSection).ThenInclude(x => x.Teacher).FirstOrDefaultAsync(x => x.Id == id, ct) ?? throw new KeyNotFoundException("Session khong ton tai.");
        var substitution = await db.SessionSubstitutionsSet.AsNoTracking().Where(x => x.SessionId == id && x.Status == "ACTIVE").Select(x => new { x.SubstituteTeacherId, SubstituteTeacher = x.SubstituteTeacher.FullName, x.Reason, x.Status }).FirstOrDefaultAsync(ct);
