...
public sealed class SearchController(AppDbContext db):ControllerBase
{
 ...
 {
  var key=q?.Trim()??"";
  var tid=await AccessScope.TeacherIdAsync(db,User,ct);var sessionsQ=AccessScope.Sessions(db,User,tid).AsNoTracking().Include(x=>x.ClassSection).AsQueryable();var permittedSessionIds=sessionsQ.Select(x=>x.Id);var permittedClassIds=sessionsQ.Select(x=>x.ClassSectionId);var permittedStudentIds=db.SessionStudentsSet.Where(x=>permittedSessionIds.Contains(x.SessionId)).Select(x=>x.StudentId);
  var students=await db.StudentsSet.AsNoTracking().Where(x=>x.IsActive&&permittedStudentIds.Contains(x.Id)&&(key==""||x.StudentCode.Contains(key)||x.FullName.Contains(key))).OrderBy(x=>x.StudentCode).Take(30).Select(x=>new{x.Id,x.StudentCode,x.FullName}).ToListAsync(ct);
  var classesQ=db.ClassSections.AsNoTracking().Where(x=>permittedClassIds.Contains(x.Id));if(key!="")classesQ=classesQ.Where(x=>x.Code.Contains(key)||x.Name.Contains(key));var classes=await classesQ.OrderBy(x=>x.Code).Take(30).Select(x=>new{x.Id,x.Code,x.Name,x.Semester,x.AcademicYear}).ToListAsync(ct);
  if(from!=null)sessionsQ=sessionsQ.Where(x=>x.ScheduledStart>=from);if(to!=null)sessionsQ=sessionsQ.Where(x=>x.ScheduledStart<to);if(key!="")sessionsQ=sessionsQ.Where(x=>x.ClassSection.Code.Contains(key)||x.Status.Contains(key));var sessions=await sessionsQ.OrderByDescending(x=>x.ScheduledStart).Take(50).Select(x=>new{x.Id,x.ClassSectionId,ClassSection=x.ClassSection.Code,x.ScheduledStart,x.Status}).ToListAsync(ct);
