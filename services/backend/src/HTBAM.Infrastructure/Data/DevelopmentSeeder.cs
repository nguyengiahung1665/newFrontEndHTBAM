...
public static class DevelopmentSeeder
{
 public static async Task SeedAsync(AppDbContext db,CancellationToken ct=default)
 {
  ...
  await EnsureUser(db,"dean","dean@example.local","Trưởng khoa Demo","Dean@123456","LECTURER",ct);
  await EnsureUser(db,"depthead","depthead@example.local","Trưởng bộ môn Demo","DeptHead@123456","LECTURER",ct);
  await EnsureUser(db,"lecturer2","lecturer2@example.local","Giảng viên dạy thay","Lecturer2@123456","LECTURER",ct);
 ...
 private static async Task<User> EnsureUser(AppDbContext db,string userName,string email,string fullName,string password,string roleName,CancellationToken ct){var u=await db.UsersSet.Include(x=>x.UserRoles).FirstOrDefaultAsync(x=>x.UserName==userName,ct);if(u is null){u=new User{UserName=userName,Email=email,FullName=fullName,Status="ACTIVE"};u.PasswordHash=new PasswordHasher<User>().HashPassword(u,password);db.UsersSet.Add(u);await db.SaveChangesAsync(ct);}else if(u.PasswordHash=="DEV-SAMPLE-NOT-A-LOGIN-HASH"){u.PasswordHash=new PasswordHasher<User>().HashPassword(u,password);u.TokenVersion++;await db.SaveChangesAsync(ct);}var role=await db.RolesSet.SingleAsync(x=>x.Name==roleName,ct);if(!await db.UserRoles.AnyAsync(x=>x.UserId==u.Id&&x.RoleId==role.Id,ct)){db.UserRoles.Add(new UserRole{UserId=u.Id,RoleId=role.Id});await db.SaveChangesAsync(ct);}return u;}
